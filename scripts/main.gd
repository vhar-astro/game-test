...
func _setup_input() -> void:
	var keys := {"move_forward":KEY_W,"move_back":KEY_S,"move_left":KEY_A,"move_right":KEY_D,"jump":KEY_SPACE,"sprint":KEY_SHIFT,"interact":KEY_E,"resonance":KEY_1,"pause":KEY_ESCAPE}
	for action in keys:
...
func _load_world(id: String, restore_position: bool) -> void:
	...
	if id=="forest" and not session.robot_defeated:
		...
		robot.attack_hit.connect(func(amount): player.take_damage(amount))
	...
	audio.clear_spatial()
	if id=="forest":
		audio.loop_spatial("ship",Vector3(-5.6,0,14))
...
func _set_mode(value: String) -> void:
	...
	if is_instance_valid(player): player.input_enabled=active
	...
	if is_instance_valid(world): world.set_process(mode in ["explore","menu","puzzle"])
...
func _unhandled_input(event: InputEvent) -> void:
	...
	elif mode=="explore" and event.is_action_pressed("interact") and not settings.interaction_hold:
		interact(nearest_id)
		get_viewport().set_input_as_handled()

func _process(delta: float) -> void:
	if mode not in ["explore","puzzle"]: return
	...
	var bounds:=Vector2(11,31) if session.scene_id=="forest" else Vector2(9,14) if session.scene_id=="ruins" else Vector2(9,9)
	var center_z:=-13.0 if session.scene_id=="forest" else -4.0 if session.scene_id=="ruins" else 0.0
	var edge_distance:=minf(bounds.x-absf(player.position.x),bounds.y-absf(player.position.z-center_z))
	...
	if player.is_on_floor() and absf(player.global_position.x)<10.3:
		last_safe=player.global_position
	...
	if settings.interaction_hold and Input.is_action_pressed("interact") and not nearest_id.is_empty():
		hold_elapsed+=delta
...
func travel(destination: String) -> bool:
	if mode!="explore" or not DIMENSIONS.has(destination): return false
	...
	session.player_position=world.spawn_position
...
func _on_command(action: String, value: Variant = null) -> void:
	match action:
		...
		"resume": _set_mode("explore"); ui.show_hud()
		...
		"hub": _set_mode("explore"); travel("hub")
